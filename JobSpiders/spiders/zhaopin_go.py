...
class ZhaopinGoSpider(scrapy.Spider):
    name = 'zhaopin_go'
    ...
    start_urls = ['https://fe-api.zhaopin.com/c/i/sou?pageSize=60&cityId=489&workExperience=-1&education=-1'
                  '&companyType=-1&employmentType=-1&jobWelfareTag=-1&kw=go&kt=3&lastUrlQuery=%7B%22jl%22:%22489%22,'
                  '%22kw%22:%22go%22,%22kt%22:%223%22%7D']
    ...
    def parse_detail(self, response):
        ...
        if response.status == 200 and response.meta.get('meta_data', '') < (json.loads(response.text))['data'][
            ...
            url_next = 'https://fe-api.zhaopin.com/c/i/sou?start={start}&pageSize=60&cityId=489&workExperience=-1&education=-1&companyType=-1&employmentType=-1&jobWelfareTag=-1&kw=go&kt=3&lastUrlQuery={lastUrlQuery}'.format(
