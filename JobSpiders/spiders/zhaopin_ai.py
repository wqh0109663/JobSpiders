...
class ZhaopinAiSpider(scrapy.Spider):
    ...
    start_urls = ['https://fe-api.zhaopin.com/c/i/sou?pageSize=60&cityId=489&workExperience=-1&education=-1'
                  '&companyType=-1&employmentType=-1&jobWelfareTag=-1&kw=%E4%BA%BA%E5%B7%A5%E6%99%BA%E8%83%BD&kt=3'
                  '&lastUrlQuery=%7B%22jl%22:%22489%22,%22kw%22:%22%E4%BA%BA%E5%B7%A5%E6%99%BA%E8%83%BD%22,'
                  '%22kt%22:%223%22%7D']
    ...
    def parse_detail(self, response):
        ...
        if response.status == 200 and response.meta.get('meta_data', '') < (json.loads(response.text))['data'][
            ...
            dic_page = {"p": 1, "jl": "489", "kw": "人工智能", "kt": "3"}
            dic_page['p'] = page
            data = '{0}'.format(dic_page)
            print(data)
            from urllib import parse
            url_data = parse.quote(string=data, encoding="utf-8")
            url_next = 'https://fe-api.zhaopin.com/c/i/sou?start={start}&pageSize=60&cityId=489&workExperience=-1&education=-1&companyType=-1&employmentType=-1&jobWelfareTag=-1&kw=%E4%BA%BA%E5%B7%A5%E6%99%BA%E8%83%BD&kt=3&lastUrlQuery={lastUrlQuery}'.format(
